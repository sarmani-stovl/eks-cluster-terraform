data "template_file" "launch_template_userdata_node_group_1" {
...
resource "random_id" "worker-node-group-1" {
  keepers = {
...
resource "aws_launch_template" "worker-node-group-1" {
  name_prefix            = "eks-worker-node-group-1-"
  description            = "worker-node-group-1 launch-Template"
  ...
  user_data = base64encode(
    data.template_file.launch_template_userdata_node_group_1.rendered,
  )
  ...
  tag_specifications {
    ...
    tags = {
      	Name      = "${local.name}-worker-${random_id.worker-node-group-1.hex}"
